...
function UnlockDialog({
  ...
  const unlockAvailableFrom = memory.unlockAvailableFrom
    ? new Date(memory.unlockAvailableFrom)
    : null;
      ...
      <div className="modal-content" onClick={(event) => event.stopPropagation()}>
        ...
        {memory.unlockRequiresLocation && (
          <p className="memory-card__meta">
            <span className="memory-radiius flex gap-2 items-center">
        ...
        {requiresPasscode && (
          <div style={{ marginTop: '0.75rem' }}>
            <p className="muted" style={{ margin: '0 0 0.25rem' }}>This memory requires a passcode.</p>
            <input
              type="password"
              value={passcode}
        ...
        {unlockAvailableFrom && (
          <p className="muted" style={{ marginTop: '0.5rem' }}>
            Unlocks from{' '}
            {unlockAvailableFrom.toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })}
          </p>
        )}
